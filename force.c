...
void ffL_set_calcffandforce(struct potential *ene, struct force *f, struct AmberParmL AP){
  ...
  ffL_set_non_bonding_index_1(&numnb,&num14,AP);
  (*ene).parm.numnb=numnb;
  (*ene).parm.num14=num14;
  (*ene).parm.indexnb=(int *)emalloc(sizeof(int)*numnb*2);
  (*ene).parm.index14=(int *)emalloc(sizeof(int)*num14*2);
  ffL_set_non_bonding_index_2((*ene).parm.indexnb,(*ene).parm.index14,AP);

  numatom=AP.NATOM;
  (*ene).parm.ele=(double *)emalloc(sizeof(double)*numatom);
  (*ene).parm.ALJ=(double *)emalloc(sizeof(double)*numatom*numatom);
  (*ene).parm.BLJ=(double *)emalloc(sizeof(double)*numatom*numatom);
  (*ene).p_e=(double *)emalloc(sizeof(double)*numnb);
  (*ene).p_LJ=(double *)emalloc(sizeof(double)*numnb);
  (*ene).p_e_14=(double *)emalloc(sizeof(double)*numnb);
  (*ene).p_LJ_14=(double *)emalloc(sizeof(double)*numnb);
  (*ene).p_d=(double *)emalloc(sizeof(double)*(AP.NPHIH+AP.MPHIA));
  (*ene).p_a=(double *)emalloc(sizeof(double)*(AP.NTHETH+AP.MTHETA));
  (*ene).p_b=(double *)emalloc(sizeof(double)*(AP.NBONH+AP.MBONA));

  ffL_set_NB_PARM((*ene).parm.ele,(*ene).parm.ALJ,(*ene).parm.BLJ,numatom,AP);

  (*f).f_t=(double *)emalloc(sizeof(double)*numatom*3);
  (*f).f_e=(double *)emalloc(sizeof(double)*numatom*3);
  (*f).f_LJ=(double *)emalloc(sizeof(double)*numatom*3);
  (*f).f_e_14=(double *)emalloc(sizeof(double)*numatom*3);
  (*f).f_LJ_14=(double *)emalloc(sizeof(double)*numatom*3);
  (*f).f_d=(double *)emalloc(sizeof(double)*numatom*3);
  (*f).f_a=(double *)emalloc(sizeof(double)*numatom*3);
  (*f).f_b=(double *)emalloc(sizeof(double)*numatom*3);

}

void ffL_free(struct potential ene, struct force f){

  free(ene.parm.indexnb);
  free(ene.parm.index14);
...
void ffL_set_non_bonding_index_1(int *numindexnb, int *numindex14, struct AmberParmL AP) {
  ...
  for(i=0;i<numatom;++i) {
    for(j=i+1;j<numatom;++j) {
      if (which_calc_nb(i,j,AP) == YES)
	++gnumnb;
      else if(which_calc_14_nb(i,j,AP)==YES)
      	++gnum14;
    }
  }
...
int which_calc_14_nb(int atomi,int atomj, struct AmberParmL AP) {
  ...
  if ((flag=which_calc_nb(atomi,atomj,AP))==YES)
    return NO;
  else {
...
double ffL_calcffandforce(double *crd, int numatom,
  ...
  ffL_calcDIHE((*ene).p_d,n_d,crd,1,0,0,AP);
  ffL_calcDIHE_force_Cartesian((*f).f_d,crd,AP); // 1111

  ffL_calcANGLE((*ene).p_a,crd,AP);
  ffL_calcANGLE_force_Cartesian((*f).f_a,crd,AP);

  ffL_calcBOND((*ene).p_b,crd,AP);
  ffL_calcBOND_force_Cartesian((*f).f_b,crd,AP);
...
void ffL_calcDIHE_force_Cartesian(double *f_d,double *cord, struct AmberParmL AP) {
  ...
  n1=(double *)emalloc(sizeof(double)*3);
  n2=(double *)emalloc(sizeof(double)*3);
...
int ffL_calcANGLE_force_Cartesian(double *f_a,double *cord, struct AmberParmL AP){
  ...
  f_temp=(double *)emalloc(sizeof(double)*3*3);
  ...
  for (i=0;i<AP.NTHETH;++i) {
    ...
    kang = AP.TK[type];
    ang_eq = AP.TEQ[type];
    calcANGKE_force(atom[0],atom[1],atom[2],kang,ang_eq,f_temp,AP);
    for (j=0;j<3;++j) {
...
void calcANGKE_force(double atomi[3],double atomj[3],double atomk[3],double kang,double ang_eq,double *f, struct AmberParmL AP) {
  int i,j,k;
  double lenij,lenkj;
  double vij[3],vkj[3];
